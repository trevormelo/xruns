...
# =============================================================================
# MASTER TOGGLE — run value reliability weights
# -----------------------------------------------------------------------------
# Set to TRUE  → apply sabermetric reliability scalars to baserunning and
#                 fielding run values (BASERUNNING_RELIABILITY = 0.70,
#                 FIELDING_RELIABILITY = 0.50), dampening single-season noise
#                 in these raw Statcast components.
# Set to FALSE → baserunning and fielding run values are used at full weight
#                 (scalars set to 1.0), treating them the same as hitting and
#                 pitching run values with no additional dampening.
# =============================================================================
USE_RUN_VALUE_WEIGHTS <- FALSE
...
#
# Controlled by USE_RUN_VALUE_WEIGHTS above. Set to FALSE to use full weight (1.0).
if (USE_RUN_VALUE_WEIGHTS) {
  FIELDING_RELIABILITY    <- 0.50
  BASERUNNING_RELIABILITY <- 0.70
} else {
  FIELDING_RELIABILITY    <- 1.0
  BASERUNNING_RELIABILITY <- 1.0
}
...
load_year <- function(base, y) {
  ...
  # Attach fielding to hitter/fielder rows. For dual-role players (e.g. Ohtani),
  # attribute their fielding totals to one role only, based on the larger PA sample.
  # Pitcher-only fielding rows are excluded so a pitcher-oriented export cannot
  # inflate pitcher Overall/xRuns in the roster table.
  if (nrow(fld_df) > 0) {
    ...
    pit_fld <- fld_df %>% filter(player_id %in% intersect(fld_pit_ids, dual_as_pitcher))
